...
function App() {
  ...
  const handleComplete = (listIndex, taskId) => {
    setTodoLists((prevLists) => {
      const updatedTodoLists = [...prevLists];
      const task = updatedTodoLists[listIndex].tasks.find((task) => task.id === taskId);
      ...
      updatedTodoLists[listIndex].tasks.sort((a, b) => (a.complete === b.complete ? 0 : a.complete ? -1 : 1));
      return updatedTodoLists;
    });
  };
  ...
  const handleRemoveTodoList = (listIndex) => {
    ...
    setCompletedTaskCount((prevCount) => {
      const completedTasksToRemove = tasksToRemove.filter((task) => task.complete).length;
    ...
    <div className="container">
      <div className="header">
        ...
        {todoLists.length <= maxTodoLists && (
          <button className="todoBtn" onClick={addTodoList}>Add Todo List</button>
        )}
      </div>
        ...
        {todoLists.map((todoList, listIndex) => (
          <div className="todoCard" key={todoList.id}>
            <div className="todoHeader">
              ...
              <button style={{ background: "none", border: "none" }} onClick={() => handleRemoveTodoList(listIndex)}>
                <a style={{ color: "lightsalmon", fontSize: "14px" }} href="#">Close</a>
              </button>
            </div>
            <input
              type="text"
            ...
            <span>
              {errors[listIndex] && <div style={{ color: "#ff5353" }}>{errors[listIndex]}</div>}
            </span>
            <table>
              <thead>
                <tr>
      ...
      <div className="footer">
        ...
        <div>Total Pending Tasks: {todoLists.reduce((total, list) => total + list.tasks.length - list.tasks.filter((task) => task.complete).length, 0)}</div>
      </div>
    </div>
  );
}
